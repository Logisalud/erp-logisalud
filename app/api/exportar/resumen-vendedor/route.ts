...
export async function GET(_req: NextRequest) {
  try {
    ...
    type Grupo = {
      ...
      clientes: Set<string>;
      saldo_total: number; vigente: number; d1_30: number; d31_60: number;
      d61_90: number; mas90: number; cant_facturas: number;
    };
    ...
    for (const row of data ?? []) {
      ...
      if (!map.has(key)) {
        map.set(key, {
          ...
          clientes: new Set(),
          saldo_total: 0, vigente: 0, d1_30: 0, d31_60: 0, d61_90: 0, mas90: 0,
          cant_facturas: 0,
        });
      }
      const g = map.get(key)!;
    ...
    const rows = Array.from(map.values())
      ...
      .map(g => {
        const vencido = g.d1_30 + g.d31_60 + g.d61_90 + g.mas90;
        return {
    ...
    ws['!cols'] = [
      ...
      { wch: 13 }, { wch: 11 }, { wch: 11 }, { wch: 11 }, { wch: 11 }, { wch: 11 },
      { wch: 13 }, { wch: 12 },
    ];
