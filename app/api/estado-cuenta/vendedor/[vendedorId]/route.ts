...
export async function GET(
  ...
  try {
    ...
    const { data, error } = await (vendedorId === 'sin-asignar'
      ? base.is('vendedor_id', null)
      : base.eq('vendedor_id', vendedorId));
    ...
    for (const row of (data as SaldoRow[])) {
      ...
      switch (row.rango_vencimiento) {
        case 'vigente':
        case 'sin_vencimiento': g.vigente += s; break;
    ...
    const clientes = Array.from(grupos.values())
