...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await db
    .from('v_saldos')
    .select('cliente_ruc, razon_social, vendedor_nombre, vendedor_codigo, zona_nombre, saldo_pendiente, vigente, d1_30, d31_60, d61_90, mas90')
  ...
  for (const row of data ?? []) {
    ...
    c.d1_30        += Number(row.d1_30)           || 0;
    c.d31_60       += Number(row.d31_60)          || 0;
    c.d61_90       += Number(row.d61_90)          || 0;
    c.mas90        += Number(row.mas90)           || 0;
