...
export interface FilaNubefact {
  ...
  fila_excel: number;           // para reporte de errores
}
...
export interface ResumenPreview {
  ...
  anuladas: number;
  clientes_nuevos: string[];    // RUCs que no existen aún
...
function parseFechaDesdeDetalle(detalle: string): string | null {
  // "POR PAGAR [CRÉDITO] - 283.94 - 18/07/2026"
...
function parseTipo(raw: unknown): string {
  const s = String(raw ?? '').trim();
  // Nubefact puede traer '01', 'FACTURA', '07', 'NOTA DE CRÉDITO', etc.
...
export function parsearExcelNubefact(buffer: ArrayBuffer): FilaNubefact[] {
  ...
  // Convertir a arreglo de objetos usando la primera fila como headers
  ...
  raw.forEach((row, idx) => {
    const fila = idx + 2; // fila Excel (1-indexed + header)

    const tipo = parseTipo(row['TIPO']);
    const serie = String(row['SERIE'] ?? '').trim().toUpperCase();
    const numeroRaw = Number(row['NÚMERO'] ?? row['NUMERO'] ?? 0);
    ...
    const docModTipo = row['DOC MODIFICADO - TIPO']
      ? parseTipo(row['DOC MODIFICADO - TIPO'])
      : null;
    const docModSerie = row['DOC MODIFICADO - SERIE']
      ? String(row['DOC MODIFICADO - SERIE']).trim().toUpperCase()
      : null;
    const docModNumero = row['DOC MODIFICADO - NÚMERO'] ?? row['DOC MODIFICADO - NUMERO']
      ? Number(row['DOC MODIFICADO - NÚMERO'] ?? row['DOC MODIFICADO - NUMERO'])
      : null;
