-- Las bonificaciones (código `BO…`) se pueden cargar a mano, a S/ 0.00.
--
-- El buscador de productos no las ofrecía porque `esOfrecibleEnPedido`
-- exigía precio vigente, y casi ninguna lo tiene: se entregan gratis. Pero
-- el bloqueo de verdad estaba acá: `submit_order` levantaba "Sin precio
-- vigente para el producto X" y tumbaba el envío ENTERO por una sola línea
-- de bonificación.
--
-- Excepción angosta y explícita: sólo los códigos `BO…`. Para cualquier
-- otro producto, "no tiene precio" es un dato que falta y la línea se
-- bloquea, en vez de valorizarse en cero por accidente. El espejo en
-- TypeScript vive en domain/products.ts (`admitePrecioCero`).
--
-- Esto NO es el motor de promociones automáticas (escalas, "3+1"), que
-- sigue pendiente de diseño: sólo permite cargar a mano una bonificación
-- ya acordada con el cliente.
--
-- Se reescribe la función entera porque el cambio cae dentro de su cuerpo;
-- el resto es idéntico a 1012.
