...
const NAV_LINKS = [{ href: "/operaciones", label: "Despachos" }];

export default async function OperacionesLayout({ children }: { children: React.ReactNode }) {
  const user = await requireRole(["operaciones", "administrador"]);
      ...
      <header className="sticky top-0 z-10 border-b border-gray-200 bg-white">
        <div className="mx-auto flex max-w-4xl flex-wrap items-center justify-between gap-3 p-4">
          <div className="flex flex-wrap items-center gap-4">
            <Link href="/operaciones" className="text-lg font-bold text-logisalud-green">
              LOGISALUD Pedidos
            </Link>
            <nav className="flex flex-wrap gap-3 text-sm">
              {NAV_LINKS.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="hover:text-logisalud-green hover:underline"
                >
                  {link.label}
                </Link>
              ))}
