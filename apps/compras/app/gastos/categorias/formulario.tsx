...
import { crearCategoriaAction, type EstadoFormulario } from './actions'

export function FormularioCategoria() {
  const [estado, accion] = useFormState<EstadoFormulario, FormData>(crearCategoriaAction, null)
  ...
  return (
    <form action={accion} className="card space-y-3">
      ...
      <label className="block text-sm">
        <span className="font-medium text-gray-800">Cuenta contable (opcional)</span>
        <input
          type="text" name="cuentaContable"
          className="mt-1 min-h-12 w-full rounded-md border border-gray-300 px-3"
        />
      </label>
